...
private QueueConnection con;
    ...
    private void initJms(){
        try {
            ...
            queue_ds = (Queue) ctx.lookup("chat/queueDestination1");
            queue_tg = (Queue) ctx.lookup("chat/queueDestination2");
            con = (QueueConnection)factory.createConnection();
            con.start();
            session = con.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
            sender = session.createSender(queue_ds);
            receiver = session.createReceiver(queue_tg, "destination = 2");
            message = session.createTextMessage();
            receiver.setMessageListener((Message msg) -> {
                TextMessage tmp = (TextMessage) msg;
                try {
                    jTextArea1.append("Client 2:" + tmp.getText() + "\n");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if(! msg.equals("")){
            try {
                ...
                message.setIntProperty("destination", 1);
                jTextArea2.setText("");
                sender.send(message);
            } catch (JMSException ex) {
                Logger.getLogger(Client1.class.getName()).log(Level.SEVERE, null, ex);
